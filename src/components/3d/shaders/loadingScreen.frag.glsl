/**
 * Loading screen fragment shader adapted from lusion-reverse-engineered-main.
 * Draws a progress bar and animated "L"-shaped letter reveal with post-load sequence.
 */
...
vec2 rotate2d(vec2 pos, float radians) {
    float c = cos(radians);
    float s = sin(radians);
...
vec2 rotateAroundAnchor(vec2 pos, vec2 anchor, float radians) {
    return rotate2d(pos - anchor, radians) + anchor;
}
...
void main() {
    ...
    float letterRotationCurve = smoothstep(0.0, 0.1, postLoadSequenceProgress);
    ...
    ndcUv = rotateAroundAnchor(ndcUv, vec2(0.0), mix(0.0, PI / 16.0, scaleAndRotateCurve));
    ndcUv += mix(vec2(0.0), vec2(0.0, 0.1), scaleAndRotateCurve);

    vec2 verticalUv = rotateAroundAnchor(ndcUv, Letter1RotateAnchor, letterRotation);
